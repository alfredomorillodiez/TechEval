...
Al abrir la ruta `/exam/{Token}`, la interfaz SHALL validar el token contra el backend antes de mostrar ningún contenido del examen, y SHALL presentar una pantalla de bienvenida con el título del examen, el nombre del candidato, el número de preguntas y el tiempo límite únicamente cuando el token resulte válido.

#### Scenario: Token válido muestra información del examen
- **WHEN** el candidato abre un enlace con un token que existe, no ha expirado y no ha sido usado
- **THEN** la interfaz muestra la pantalla de bienvenida con el título del examen, el nombre del candidato, el número de preguntas y el tiempo límite en minutos, junto con un botón para comenzar
...
- **THEN** la interfaz muestra una pantalla de error con el mensaje devuelto por el backend y SHALL NOT mostrar la pantalla de bienvenida ni ninguna pregunta del examen

### Requirement: Inicio del examen con temporizador visible
Al pulsar "Comenzar examen" desde la pantalla de bienvenida, la interfaz SHALL iniciar la sesión de examen contra el backend y SHALL mostrar un temporizador en cuenta atrás visible en todo momento durante la resolución del examen.

#### Scenario: Inicio de sesión arranca el temporizador
- **WHEN** el candidato pulsa "Comenzar examen"
- **THEN** la interfaz solicita el inicio de sesión al backend, transiciona a la pantalla de preguntas y arranca un temporizador en cuenta atrás inicializado con el tiempo límite del examen en minutos
...
Durante la resolución del examen, la interfaz SHALL renderizar cada pregunta según su tipo — opciones de selección única para preguntas tipo test, o un área de texto libre para preguntas abiertas — y SHALL permitir navegar libremente entre preguntas sin perder las respuestas ya introducidas.
...
La interfaz SHALL enviar cada respuesta al backend en el momento en que el candidato la registra, sin esperar al envío final del examen: inmediatamente al seleccionar una opción en preguntas tipo test, y al perder el foco del campo de texto en preguntas abiertas.
...
La interfaz SHALL enviar automáticamente el examen completo, con todas las respuestas registradas hasta ese momento, en el instante en que el temporizador alcanza cero, sin requerir confirmación del candidato.

#### Scenario: El tiempo se agota durante la resolución
- **WHEN** el temporizador en cuenta atrás llega a cero mientras el candidato aún está respondiendo el examen
- **THEN** la interfaz detiene el temporizador y envía automáticamente el examen con el conjunto de respuestas disponible en ese momento, sin mostrar el diálogo de confirmación manual

### Requirement: Envío manual antes de agotar el tiempo
La interfaz SHALL permitir al candidato finalizar y enviar el examen manualmente en cualquier momento antes de que se agote el tiempo, mostrando previamente un diálogo de confirmación con el número de preguntas respondidas.

#### Scenario: Candidato solicita finalizar desde la última pregunta
- **WHEN** el candidato pulsa "Finalizar examen" estando en la última pregunta y con tiempo restante
...
Tras un envío exitoso (manual o automático), la interfaz SHALL mostrar una pantalla final con el resultado obtenido y un aviso de que el detalle llegará también por email, sin permitir volver a las preguntas del examen.

#### Scenario: Examen enviado muestra el resultado
- **WHEN** el envío del examen se completa correctamente
